...
class SoftNotes extends StatefulWidget {
  @override
  _SoftNotesState createState() => _SoftNotesState();
}

class _SoftNotesState extends State<SoftNotes> {
  ...
  void initState() {
    ...
    getFileFromAsset1("assets/sc1.pdf").then((f){
      setState(() {
        assetPDF1Path = f.path;
        print(assetPDF1Path);
      });
    });
    getFileFromAsset2("assets/sc2.pdf").then((f){
      setState(() {
  ...
  Future<File> getFileFromAsset1(String asset)async{
    try{
      ...
      File file = File("${dir.path}/sc1.pdf");
  ...
  Future<File> getFileFromAsset2(String asset)async{
    try{
      ...
      var bytes = data.buffer.asUint8List();
      var dir = await getApplicationDocumentsDirectory();
      File file = File("${dir.path}/sc2.pdf");
      File assetFile = await file.writeAsBytes(bytes);
      return assetFile;
    }catch(e){
      throw Exception("Error");
