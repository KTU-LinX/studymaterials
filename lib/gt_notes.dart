...
class GtNotes extends StatefulWidget {
  @override
  _GtNotesState createState() => _GtNotesState();
}

class _GtNotesState extends State<GtNotes> {
  ...
  void initState() {
    ...
    getFileFromAsset1("assets/gt1.pdf").then((f){
      setState(() {
        assetPDF1Path = f.path;
        print(assetPDF1Path);
      });
    });
    getFileFromAsset2("assets/gt2.pdf").then((f){
      setState(() {
  ...
  Future<File> getFileFromAsset1(String asset)async{
    try{
      ...
      File file = File("${dir.path}/gt1.pdf");
  ...
  Future<File> getFileFromAsset2(String asset)async{
    try{
      ...
      var bytes = data.buffer.asUint8List();
      var dir = await getApplicationDocumentsDirectory();
      File file = File("${dir.path}/gt2.pdf");
      File assetFile = await file.writeAsBytes(bytes);
      return assetFile;
    }catch(e){
      throw Exception("Error");
