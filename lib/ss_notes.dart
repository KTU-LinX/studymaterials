...
class SsNotes extends StatefulWidget {
  @override
  _SsNotesState createState() => _SsNotesState();
}

class _SsNotesState extends State<SsNotes> {
  ...
  void initState() {
    ...
    getFileFromAsset1("assets/ss1.pdf").then((f){
      setState(() {
        assetPDF1Path = f.path;
        print(assetPDF1Path);
      });
    });
    getFileFromAsset2("assets/ss2.pdf").then((f){
      setState(() {
  ...
  Future<File> getFileFromAsset1(String asset)async{
    try{
      ...
      File file = File("${dir.path}/ss1.pdf");
  ...
  Future<File> getFileFromAsset2(String asset)async{
    try{
      ...
      var bytes = data.buffer.asUint8List();
      var dir = await getApplicationDocumentsDirectory();
      File file = File("${dir.path}/ss2.pdf");
      File assetFile = await file.writeAsBytes(bytes);
      return assetFile;
    }catch(e){
      throw Exception("Error");
