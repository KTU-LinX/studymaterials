...
class MmNotes extends StatefulWidget {
  @override
  _MmNotesState createState() => _MmNotesState();
}

class _MmNotesState extends State<MmNotes> {
  ...
  void initState() {
    ...
    getFileFromAsset1("assets/mm1.pdf").then((f){
      setState(() {
        assetPDF1Path = f.path;
        print(assetPDF1Path);
      });
    });
    getFileFromAsset2("assets/mm2.pdf").then((f){
      setState(() {
  ...
  Future<File> getFileFromAsset1(String asset)async{
    try{
      ...
      File file = File("${dir.path}/mm1.pdf");
  ...
  Future<File> getFileFromAsset2(String asset)async{
    try{
      ...
      var bytes = data.buffer.asUint8List();
      var dir = await getApplicationDocumentsDirectory();
      File file = File("${dir.path}/mm2.pdf");
      File assetFile = await file.writeAsBytes(bytes);
      return assetFile;
    }catch(e){
      throw Exception("Error");
